...
//		List<?> gList = spatialIndex.queryAll();
    ...
    // added by Lishan.
    public Bag getClosestObject(final Geometry g)
    {
    	Bag nearbyObjects = new Bag();
        for (double dist = 0.00005; dist < 0.0011; dist = dist + 0.00001)
        {
            Envelope e = g.getEnvelopeInternal();
            e.expandBy(dist);
...
//        System.out.println(g + " "+ nearbyObjects.size());
        return nearbyObjects;
    }


    // added by Lishan.
    public Bag getClosestObject(final MasonGeometry mg)
    {
        return getClosestObject(mg.getGeometry());
    }
...
//             spatialIndex.insert(g1.geometry.getEnvelopeInternal(), g1);
        ...
        {
        	System.out.println("g1 null set geometryloc");
        }

//        System.out.println("in setgeoloc " + g );
        needToRebuildIndex = true;
    }

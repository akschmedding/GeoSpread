...
import sim.app.geo.covidsimulationisolationcross.Agent;
 ...
 * push it up.
 */
public class CovidSimulationIsolationCross extends SimState {
    ...
    public static void main(String[] args){
        doLoop(CovidSimulationIsolationCross.class, args);
        System.exit(0);
    }
     ...
     * Constructor
     */
    public CovidSimulationIsolationCross(long seed) {
        super(seed);
    }
        ...
        try {
            ...
            Steppable flipper = new Steppable()
            {
                ...
                public void step(SimState state) {
                	...
                	CovidSimulationIsolationCross gstate = (CovidSimulationIsolationCross) state;
            ...
            Logger.getLogger(CovidSimulationIsolationCross.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(CovidSimulationIsolationCross.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void infect(ArrayList<Agent> curAgents) {
    	if (curAgents.get(0).atHome) { //infection at home
    		...
    		for (ArrayList<Agent> f: families) {
    			for (Agent a : f) {
    				if (a.atHome && a.checkSymptomatic()) {
    					infected++;
    				}
    				if (a.atHome) {
    					curHome.add(a);

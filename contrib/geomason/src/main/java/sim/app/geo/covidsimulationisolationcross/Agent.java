 ...
 * $Id$
 */
package sim.app.geo.covidsimulationisolationcross;
...
public final class Agent implements Steppable {
    ...
    private boolean symptomatic = false; // flag of whether this agent is symptomatic
    public ArrayList<Agent> family = new ArrayList<Agent>();
    public int area = -1;
    ...
    CovidSimulationIsolationCross world;
    ...
    public ArrayList<Double> xActivityLength= new ArrayList<Double>();
    public ArrayList<Double> yActivityLength= new ArrayList<Double>();
    ...
    public Agent(CovidSimulationIsolationCross g) {
        world = g;
     ...
     */
    public boolean start(CovidSimulationIsolationCross state) {
    ...
    public void step(SimState state) {
    	if (inBuilding == true) {
    		...
    		if (mv_cycle > 0) {
    			...
    			stay_cycle = (int) Math.ceil(sampleFromDistr(xActivityLength, yActivityLength)*60.0/5.0); //5min cycles
    ...
    public boolean checkSymptomatic() {
        return symptomatic;
    }
    public void setSymptomatic() { // for return value: true - success; false - failed
		System.out.println("in Agent: " +  this + " symptomatic.");
    	symptomatic = true;
    }
    ...
    public void setSpeed (ArrayList<Double> xAxis, ArrayList<Double> yAxis) {
    	double s_sampled = sampleFromDistr(xAxis, yAxis);
    	speed = s_sampled/12.0;
    	move_speed = s_sampled/12.0;
    }
    ...
    public Coordinate chooseDestination() { // return the id of the destination edge.
    	...
    	if (r>move_probability) {
    		...
    		//System.out.println(nextType + String.valueOf(nextArea));
    	...
    	setSpeed(world.xAxisSpeed,world.yAxisSpeed); //update the speed
...
//  }
  ...
  public void set_symptom_move_countdown(ArrayList<Double> xAxisSymptomMove, ArrayList<Double> yAxisSymptomMove) {
  	...
  	symptom_move_countdown = (symptom_move_countdown + 1) * 288;

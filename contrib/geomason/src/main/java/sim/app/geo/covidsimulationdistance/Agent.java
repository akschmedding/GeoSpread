...
public final class Agent implements Steppable
{
    ...
    public double move_probability = 0.9;
    // probability of leaving a building when an agent is inside the building.

    private static final long serialVersionUID = -1113018274619047013L;
    CovidSimulationDistance world;
    
    // Residence/Work Attributes
    Coordinate destination_building;
    
    Vector <Coordinate> available_buildings =  new Vector<Coordinate>();
    public Node initNode = null;
...
//    }
    ...
    public Agent(CovidSimulationDistance g, 
                 GeomPlanarGraphEdge startingEdge, double max_dist)
    {
        world = g;
        ...
        while (checkNearbyInit() == false)
        {
        	int startPos = ThreadLocalRandom.current().nextInt(world.total_edges);
            startingEdge = world.idsToEdges.get(startPos);  
            startNode = startingEdge.getDirEdge(0).getFromNode();
            initNode = startNode;
        }
     ...
     */
    public boolean start(CovidSimulationDistance state)
    {
...
//                + "of the network that cannot access the given goal node");
    ...
    private boolean findNewAStarPath(CovidSimulationDistance geoTest)
    {
    ...
    {
    	int nextPos = ThreadLocalRandom.current().nextInt(available_buildings.size());
    	Coordinate nextBuilding = available_buildings.elementAt(nextPos);
    	// maybe this is a place of logic problem. - the correctness of this function needs to be considered when debugging. 
    ...
    // this function is not used in partial lock down.
...
//    }
    ...
    public boolean checkNearbyInit()
    {
    	int count = 0;
    	Coordinate init_coord = initNode.getCoordinate();
        // go through building coordinates to find the closest

    	for (int i = 0; i < world.idsToBuildings.size(); i++) {
           Coordinate cur_coord = world.idsToBuildings.get(i);
           
           double cur_dist = getCoordDistance(init_coord, cur_coord);
           if (cur_dist < max_distance)
           {
        	   available_buildings.add(cur_coord);
        	   count ++;
           }
        }

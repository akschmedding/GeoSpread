 ...
 * $Id$
*/
package sim.app.geo.covidsimulationdistance;
 ...
 * push it up.
 */
public class CovidSimulationDistance extends SimState
{
    ...
    {
        doLoop(CovidSimulationDistance.class, args);
        System.exit(0);
    }
...
//    ArrayList<Double> yAxisMobility = new ArrayList<>();
     ...
     * Constructor
     */
    public CovidSimulationDistance(long seed)
    {
        super(seed);
    }
...
//            URL areasDB = Paths.get("data","building_demo.dbf").toUri().toURL() ;
            ...
            Steppable flipper = new Steppable()
            {
                ...
                public void step(SimState state)
                {
                	...
                	CovidSimulationDistance gstate = (CovidSimulationDistance) state;
                    ...
                    {                    	
                    		...
                    		// a is inside a building
                    	{
                    		...
                    		// print log: we find Agent a in the building.
        ...
        {
            Logger.getLogger(CovidSimulationDistance.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex)
        {
            Logger.getLogger(CovidSimulationDistance.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//            Bag found_roads = roads.getClosestObject(b);
//            // added a function getCloestObject() in GeomVectorField.
//            
//            MasonGeometry cur_road; 
//            Coordinate road_coord = new Coordinate(); 
//            
//            // this progress print out is useful when loading the whole Seoul map. It's very very very slow.
//            count_building ++;
//            if (count_building % 10000 == 0)
//            {
//            	System.out.println("building init progress " + count_building);
//            }
//
//            // the logic is that, we find the roads that are close to the building. Not accurate, but at least currently it works. 
//            // A better way is to calculate the distance of two coordinates directly. 
//            int i= 0;
//          	for (i = 0; i < found_roads.size(); i++ )
//           	{
//            	cur_road = (MasonGeometry) found_roads.objs[i];
//                road_coord = cur_road.getGeometry().getCoordinate();
//	            
//                if (edgeCoordToEdges.get(road_coord) != null)
//                	break;
//            }
//          	
//          	// just check. If happens, we need to rewrite the whole function.
//            if (edgeCoordToEdges.get(road_coord) == null)
//            {
////            	System.out.println("building "+ cur_b +" edge coor " + road_coord + " edge " + edgeCoordToEdges.get(road_coord));
//            	continue;
//            }
...
//        System.exit(0);
    ...
    public void populate()
    {
        try
        {     	
          	Scanner s = new Scanner(new File("data/dist_profiling_pop"+  Integer.toString(population) +".txt"));
            for (int i = 0; i < population; i++) 
            { 
            	double curcoordx = s.nextDouble();
            	double curcoordy = s.nextDouble();
            	int cursuperspreader = s.nextInt();
            	double curmob = s.nextDouble();
            	int curinfect = s.nextInt();
            	double curspeed = s.nextDouble();

	         	Coordinate curcoord = new Coordinate(curcoordx, curcoordy);
	
	            GeomPlanarGraphEdge startingEdge =
	            		edgeCoordToEdges.get(curcoord);  // total number of buildings: 69163
	             
	            Agent a = new Agent(this, startingEdge, max_distance);
	            
	            a.move_probability = curmob;
	            
	            if (cursuperspreader == 1 ) // 3.5897% is superspreader
	            {

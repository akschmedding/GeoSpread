...
public final class Agent implements Steppable
{
    ...
    public boolean symptom = false;
    
    public int symptom_countdown = -1; 
    // -1 means not set. 
    // If it is set, then it should be >0 (infected but no symptom), or 0 (showing symptom)
        
    private double move_probability = 0.9;
    // probability of leaving a building when an agent is inside the building.

    private static final long serialVersionUID = -1113018274619047013L;
    CovidSimulationSymptom world;
    ...
    public int symptom_move_countdown = 0; // cycles an agent can still move around after symptom_on_set
     ...
     */
    // These functions are commented by GeoMason
...
//    }
    ...
    public Agent(CovidSimulationSymptom g, 
     ...
     */
    public boolean start(CovidSimulationSymptom state)
    {
    	inBuilding = false; // agent starts move.
    	cur_stay_cycle = stay_cycle;
        if (!findNewAStarPath(state))
        {
        	//System.out.println("!findNewAStarPath(state)");
        	return false;
        }
...
//                + "of the network that cannot access the given goal node");
    ...
    private boolean findNewAStarPath(CovidSimulationSymptom geoTest)
    {
...
//        speed = progress(moveRate);
        speed = linkDirection*move_speed;
...
//        speed = progress(residualMove);
    ...
    // for return value: true - success; false - failed
    {
    	...
    	if ((symptom == false) && (symptom_countdown == -1))
    	{
    		symptom_countdown = ThreadLocalRandom.current().nextInt(12* 24, 12*24*14); // 12 cycles per hour, 1 ~14 days, uniform.
    		System.out.println("Agent="+this+" symptom_countdown="+symptom_countdown);    		
    	}
    }
    ...
    {
    	int nextPos = ThreadLocalRandom.current().nextInt(world.total_buildings);
    	Coordinate nextBuilding = world.idsToBuildings.elementAt(nextPos);
//    	while(world.buildingsToTypes.get(nextBuilding).equals("church"))
    	
    	double cur_distance = sampleFromDistr(world.xAxisDistance, world.yAxisDistance);
    	// 1 mile = 1600 meters. 
    	// max = 30 miles in distribution; we use max=3.53mile for Gangnam-gu.
    	// 30/3.53 = cur_dis (mile)/ actual_dis (mile),
    	// actual_dis (meter) = cur_dis *3.53 / 30 * 1600
    	cur_distance = cur_distance * 3.53 / 30 * 1600;
    	
    	double diff_dist = getCoordDistance(nextBuilding, startNode.getCoordinate()) - cur_distance;
    	if (diff_dist < 0)
    		// get abs(diff_dist)
    	{
    		diff_dist = -diff_dist;
    	...
    	int try_countdown = 100; // try 100 random different buildings. If we still cannot find a building, we change the target building distance.
    	while (diff_dist > 100)
    	{
    		try_countdown--;
    		if (try_countdown <= 0)
    		{
    			try_countdown = 100;
    			cur_distance = sampleFromDistr(world.xAxisDistance, world.yAxisDistance);
    			cur_distance = cur_distance * 3.53 / 30 * 1600;
    	    	
    		}
    		
    		nextPos = ThreadLocalRandom.current().nextInt(world.total_buildings);
        	nextBuilding = world.idsToBuildings.elementAt(nextPos);
        	
        	diff_dist = getCoordDistance(nextBuilding, startNode.getCoordinate()) - cur_distance;
        	if (diff_dist < 0)
        		// get abs(diff_dist)
        	{
        		diff_dist = -diff_dist;
    	...
    	// previous design using getObjectsWithinDistance.
//    	Node nextNode = nextEdge.getDirEdge(0).getToNode();
//        Coordinate nextCoord = nextNode.getCoordinate();
//        GeometryFactory fact = new GeometryFactory();
//        MasonGeometry nextLoc = new MasonGeometry(fact.createPoint(nextCoord)) ;
//        
////        for (double dist_r = 0.00008993; dist_r < 0.01; dist_r = dist_r + 0.00001)
////        {
////            Bag nearby_buildings = world.buildings.getObjectsWithinDistance(nextLoc, dist_r);
////            
////            System.out.println(this + " bag " + nearby_buildings.size() + " dist_r " + dist_r);
////
////        }
//        Bag nearby_buildings = world.buildings.getObjectsWithinDistance(nextLoc, 0.0005);
//
//        if (nearby_buildings.size() == 0)
//        {
////        	System.out.println("choose destination fin -1");
//        	return -1;
//        }
//        int rand_building = ThreadLocalRandom.current().nextInt(nearby_buildings.size());
//        MasonGeometry next_building = (MasonGeometry) nearby_buildings.objs[rand_building];
...
//    }
    ...
    public void set_symptom_move_countdown(ArrayList<Double> xAxisSymptomMove, ArrayList<Double> yAxisSymptomMove)
    {
    	symptom_move_countdown = (int) sampleFromDistr(xAxisSymptomMove, yAxisSymptomMove);
    	symptom_move_countdown = symptom_move_countdown * 12 * 24;
    	System.out.println("Agent="+this+" symptom_move_countdown="+symptom_move_countdown);
    }    
}
